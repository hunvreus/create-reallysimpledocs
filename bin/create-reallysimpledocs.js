...
const postProcess = async (targetDir) => {
  ...
  try {
    ...
    // Scaffolded projects are single-package repos.
    if (parsed?.workspaces) delete parsed.workspaces;
    if (parsed?.private) delete parsed.private;
...
const main = async () => {
  ...
  const localSourceRoot = await resolveSourceRoot();
